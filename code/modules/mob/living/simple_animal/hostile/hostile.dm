/mob/living/simple_animal/hostile
	...
	var/move_to_delay = 3
	var/movement_flags = MOVEMENT_LOOP_IGNORE_GLIDE
	...
	if(target in possible_targets)
		...
		if(retreat_distance != null) //If we have a retreat distance, check if we need to run from our target
			if(target_distance <= retreat_distance) //If target's closer than our retreat distance, run
				SSmove_manager.move_away(src, target, retreat_distance, move_to_delay, flags = movement_flags)
			else
...
/mob/living/simple_animal/hostile/proc/Goto(target, delay, minimum_distance)
	...
	SSmove_manager.move_to(src, target, minimum_distance, delay, flags = movement_flags)
